...
public class SkinSelectionController : MonoBehaviour
{
    ...
    private bool IsSkinUnlocked(int characterID, int skinID)
    {
        ...
        if (GameDataManager.Instance != null)
        {
            ...
            {
                var skinDataFromDB = characterDatabase.GetSkinByID(characterID, skinID); // Changed variable name
        ...
        var skinDataFromDatabase = characterDatabase.GetSkinByID(characterID, skinID); // Changed variable name
    ...
    private void OnSkinButtonClicked(CharacterDatabase.SkinData skinData)
    {
        ...
        Debug.Log($"Skin: {skinData.skinName} (ID: {skinData.skinID})");
        Debug.Log($"Has Timeline: {skinData.skinTimeline != null}");

        selectedSkinID = skinData.skinID;
        UpdateSkinNameDisplay();

        // ===== 1. PLAY TIMELINE & SWAP =====
        if (skinTimelineBridge != null)
        {
            // This will handle the timeline playback AND skin swapping
            skinTimelineBridge.PlayTimelineForSkin(skinData.skinTimeline, skinData.skinID);
        }
        else
        {
            // Fallback: Apply skin immediately
