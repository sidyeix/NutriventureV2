...
public class K3_NutriHeartCollector : MonoBehaviour
{
    ...
    private void InitializeCollector()
    {
        ...
        playerHealth = GetComponent<PreserviaPlayerStat>();
        
        if (playerHealth == null)
        {
            playerHealth = GetComponentInParent<PreserviaPlayerStat>();
            if (playerHealth == null)
            {
                Debug.LogError("PreserviaPlayerStat not found on player or parent! Make sure the health script is on the same GameObject or parent.");
                return;
            }
        }
        ...
        if (showDebugLogs)
        {
            Debug.Log("NutriHeart collector initialized with COIN-STYLE audio!");
