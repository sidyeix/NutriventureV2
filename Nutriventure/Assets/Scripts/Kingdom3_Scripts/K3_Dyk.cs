...
public class K3_Dyk : MonoBehaviour
{
    ...
    /// <summary>
    /// Immediately resets the DYK popup system for a game restart.
    /// Unlike ResetPopupSystem, this does a hard reset without animations.
    /// </summary>
    public void ResetDyk()
    {
        StopAllCoroutines();
        CancelInvoke();
        ...
        // Re-initialize facts and restart system
        InitializeFacts();
        Invoke(nameof(StartPopupSystem), initialDelay);

        Debug.Log("K3_Dyk: Hard reset complete.");
    }
