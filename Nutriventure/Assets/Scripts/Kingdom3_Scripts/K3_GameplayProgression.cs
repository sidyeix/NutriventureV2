...
public class K3_GameplayProgression : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // called by K3_NPCinstructions1.FinishCutscene()
    ...
    /// <summary>
    /// Explicitly set the game in-progress state. Called by NPC cutscene finish
    /// to start the game, and by restart/home flows to end it.
    /// </summary>
    public void SetGameInProgress(bool inProgress)
    {
        if (inProgress)
        {
            // Use StartGame() so the timer starts properly
            StartGame();
        }
        else
        {
