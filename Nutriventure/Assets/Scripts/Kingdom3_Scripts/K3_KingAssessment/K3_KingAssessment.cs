...
public class K3_KingAssessment : MonoBehaviour
{
    ...
    public List<int> GetCompletedFoodIndices()
    {
    ...
    public Dictionary<int, List<PreservativeType>> GetAllFoodPreservativesUsed()
    {
        return new Dictionary<int, List<PreservativeType>>(foodPreservativesUsed);
    }

    public Dictionary<int, Dictionary<PreservativeType, float>> GetAllFoodPreservationValues()
    {
        return new Dictionary<int, Dictionary<PreservativeType, float>>(foodPreservationValues);
    }

    /// <summary>
    /// Restores food completion state, particle visuals, and UI counters.
      ...
      Dictionary<int, Dictionary<PreservativeType, float>> preservationVals)
    {
        ...
        // Activate collectibles UI if anything was collected
        if (collectedPotionsCount > 0 && collectiblesParent != null && !collectiblesParent.activeSelf)
            collectiblesParent.SetActive(true);

        Debug.Log($"[K3_KingAssessment] Food state restored: {completedIndices?.Count ?? 0} foods completed.");
    }

    /// <summary>
    /// Resets all food completion state, particles, and UI counters for a fresh game.
    ...
    public void ResetForNewSession()
    {
        ...
        // Swap particles back to non-preserved
        for (int i = 0; i < foodParticles.Length; i++)
        {
            if (foodParticles[i] != null) foodParticles[i].SetActive(true);
            if (FoodPreservedPS != null && i < FoodPreservedPS.Length && FoodPreservedPS[i] != null)
                FoodPreservedPS[i].SetActive(false);
        }

        // Reset counters and UI
        collectedPotionsCount = 0;
        preservedFoodsCount = 0;
        lastCollectedCount = 0;
        if (collectedPotionCountText != null) collectedPotionCountText.text = "Collected Potion: 0/3";
        if (preservedFoodCountText != null) preservedFoodCountText.text = "Preserved Food: 0/8";
        if (collectiblesParent != null) collectiblesParent.SetActive(false);

        // Close panels
        if (KAPanel != null) KAPanel.SetActive(false);
        currentFoodIndex = -1;

        Debug.Log("[K3_KingAssessment] Reset for new session.");
    }
}