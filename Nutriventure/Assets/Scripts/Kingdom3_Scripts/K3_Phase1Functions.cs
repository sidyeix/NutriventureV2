...
public class K3_Phase1Functions : MonoBehaviour
{
    ...
    public bool IsOxidantActivated() => oxidantActivated;
    public bool IsMicrobeActivated() => microbeActivated;
    public bool IsNewRespawnPointSet() => newRespawnPointSet;

    /// <summary>
    /// Restores GEM visual state without triggering sounds or scoring.
    /// Called by K3_GameStateManager on resume.
    /// </summary>
    public void RestoreGEMState(bool oxidant, bool microbe, bool respawnSet)
    {
        if (oxidant)
        {
            oxidantActivated = true;
            ...
            if (oxidantGemRenderer != null && antiOxidantMat != null)
                oxidantGemRenderer.material = antiOxidantMat;
            if (oxidantVFX != null) oxidantVFX.SetActive(true);
            if (oxidantParticles != null)
            {
                oxidantParticles.gameObject.SetActive(true);
                oxidantParticles.Play();
            }
            if (antioxidantInfo != null) antioxidantInfo.SetActive(false);
        }
        ...
        if (microbe)
        {
            ...
            if (microbeGemRenderer != null && antiMicrobeMat != null)
                microbeGemRenderer.material = antiMicrobeMat;
            if (microbeVFX != null) microbeVFX.SetActive(true);
            if (microbeParticles != null)
            {
                microbeParticles.gameObject.SetActive(true);
                microbeParticles.Play();
            }
            if (antimicrobeInfo != null) antimicrobeInfo.SetActive(false);
        }
        ...
        if (showDebugMessages)
            Debug.Log($"[K3_Phase1Functions] GEM state restored: Oxidant={oxidant}, Microbe={microbe}, Respawn={respawnSet}");
    }
}