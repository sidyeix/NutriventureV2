...
public class K2_GameSummary : MonoBehaviour
{
    ...
    public Image panelBackgroundImage; // Assign this in inspector
    public Sprite winPanelSprite; // Assign win sprite
    public Sprite losePanelSprite; // Assign lose sprite
    ...
    public Animator starAnimator; // Assign star animator with star parameter
    public string starParameterName = "star"; // Name of the animator parameter

    [Header("Key Display")]
    public GameObject keyImageObject; // Assign GameObject with key image
    ...
    private IEnumerator ShowSummaryPanel()
    {
        ...
        if (gameSummaryPanel != null)
        {
            ...
            // Change panel sprite based on win/lose
            UpdatePanelSprite();
    ...
    // Update panel sprite based on win/lose
    private void UpdatePanelSprite()
    {
        if (panelBackgroundImage != null)
        {
            if (isVictory && winPanelSprite != null)
            {
                ...
                Debug.Log("Set panel sprite to WIN sprite");
            }
            else if (!isVictory && losePanelSprite != null)
            {
                panelBackgroundImage.sprite = losePanelSprite;
                Debug.Log("Set panel sprite to LOSE sprite");
            }
            else
            {
    ...
    // Play star animation based on number of stars earned
    private void PlayStarAnimation(int stars)
    {
        if (starAnimator != null)
        {
            starAnimator.SetInteger(starParameterName, stars);
            Debug.Log($"Playing star animation for {stars} stars");
        }
        else
        {
            Debug.LogWarning("Star animator not assigned!");
        }
    }
    ...
    private void UpdateKeyStatus(int stars)
    {
        ...
        // Show/hide key image based on key collection
        UpdateKeyImageDisplay(stars);
    }

    // Update key image display based on whether player got the key
    private void UpdateKeyImageDisplay(int stars)
    {
        if (keyImageObject != null)
        {
            // Show key image if player has 2-3 stars (key unlocked)
            bool showKey = (stars == 2 || stars == 3);
            keyImageObject.SetActive(showKey);

            Debug.Log($"Key image {(showKey ? "shown" : "hidden")} for {stars} stars");
        }
        else
        {
            Debug.LogWarning("Key image object not assigned!");
        }
    }
