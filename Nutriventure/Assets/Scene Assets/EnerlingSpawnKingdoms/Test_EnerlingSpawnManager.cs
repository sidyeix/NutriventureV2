...
public class Test_EnerlingSpawnManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnAllEnerlingsWithDelay()
    {
        ...
        Debug.Log($"[EnerlingSpawn] Filter={kingdomFilterMode}, UnlockedOnly={spawnOnlyUnlocked}, Max={maxEnerlingsInScene}, Matched={ingredientsToSpawn.Count}");

        if (ingredientsToSpawn.Count == 0)
        {
            Debug.LogError($"[EnerlingSpawn] No ingredients match filters (Kingdom={kingdomFilterMode}, UnlockedOnly={spawnOnlyUnlocked}, SpecificKingdom={specificKingdom})");
        ...
        Debug.Log($"[EnerlingSpawn] Spawned {spawnedEnerlings.Count} enerlings total");
    }
    ...
    private List<IngredientDatabase.IngredientInfo> GetFilteredIngredients()
    {
        ...
        {
            Debug.LogError("[EnerlingSpawn] Database has no ingredients!");
